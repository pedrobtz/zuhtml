...
whose measurements the roadmap cites. There is no `src/`, no vendored
Gumbo, no exported function, and the one test is the template’s. Stage 0
of the roadmap has not started.
...
- **One parent issue per target version**, titled with the bare version,
  for example `v0.1.0`. Not yet created; opening it is a Stage 0 task.
