...
It deliberately has no HTTP client, and does not execute JavaScript,
sanitize, mutate the DOM or support XPath. `html_read()` reads a path, a URL
(through base R's `url()`) or a connection; a fetcher such as `zuhttp`
hands it a string for anything more.
...
Stage 16 made `html_read()` read URLs and connections. Stage 17 (the
0.1.0 release) is next. It refreshes the CRAN preparation
...
- **One parent issue per target version**, titled with the bare version, for
  example `v0.1.0` (#2). Stages 0 to 9 are sub-issues #3 to #12, Stages
  10 to 15 are #26 to #31, Stage 16 is #40 and Stage 17 (the release) is
  #13, renumbered twice.
- **One sub-issue per roadmap stage**, titled as the roadmap titles it, for
  ...
  that section's anchor. The roadmap has eighteen stages, 0 to 17.
