...
- **Timing assertions run on CI only** (`CI=true` and not under covr):
  the local R may be emulated (x86_64 under Rosetta is about 4× slower).
...
`.github/badges/`) and `pkgdown.yml`. `hardening.yaml` runs the vendor
and lint gates with their canaries; later stages add sanitizers, fuzzing
...
- The patch series is `tools/patches/0001-max-tree-depth.patch`,
  ...
  licence-mandated (Apache-2.0 §4(b)), local only, and stays last:
  extend it whenever an earlier patch touches a new file.
  `tools/verify-vendor` fails on a modified file without the notice. The
  patch identifiers are also listed in `src/zuh_gumbo.c` and reported by
  [`zuhtml_info()`](https://pedrobtz.github.io/zuhtml/reference/zuhtml_info.md).
