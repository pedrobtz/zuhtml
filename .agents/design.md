...
*Amended at roadmap Stage 14:* raw input is now sniffed. The precedence is a BOM, then `encoding` (the transport-layer charset), then the HTML standard's prescan of the first 1024 bytes for a `<meta>` declaration, then UTF-8. This is the standard's order, except that a BOM contradicting `encoding` stays an error rather than silently winning.

The prescan (`R/sniff.R`) implements the standard algorithm step by step: comments, `<!`/`<?`/`</` constructs and tag attributes are skipped, "get an attribute", "extract a character encoding from a meta element", and the need-pragma rules. It is not a regular expression over the markup, and like browsers it does not skip script text. Labels go through the Encoding Standard's full label table, so `iso-8859-1`, `latin1` and `us-ascii` are windows-1252. A UTF-16 label means UTF-8, x-user-defined means windows-1252, and an unknown label is skipped.

Each Encoding Standard name maps to candidate `iconv()` names, supersets first (CP1252, CP932, CP949, BIG5-HKSCS), and the first one this platform's iconv accepts is used. Windows' iconv lacks a few, and there those are encoding errors, as is the "replacement" encoding and any byte sequence invalid in the declared encoding: nothing is decoded leniently. `html_info()` reports `encoding` (the Encoding Standard name for a sniffed one) and `encoding_source`, which is `"bom"`, `"argument"`, `"meta"`, `"default"`, or `"string"` for character input. HTTP headers remain the caller's to pass as `encoding`.
...
As built (roadmap Stage 12): `html_table(convert = TRUE, decimal, thousands)` replaces `col_types` with whole-column conversion, off by default so the character-first rule stands. A column converts only when every non-missing value is a logical literal or a number (a leading zero marks an identifier and keeps the column character). The grouping mark must fall between groups of three digits, and integer overflow gives a double column. A column that does not convert stays character; there is no error and no partial `NA`. `html_table_cells(x, trim, absolute, limits)` is the table above with `header` for `is_header`, a single `links` list-column of `<a>`/`<area>` hrefs outside nested tables (raw unless `absolute = TRUE`, as `html_links()`), and no node IDs or `scope`/`headers` columns. It is derived in R from the same C grid as `html_table()`, and positions come from each cell's first and last slot, so the two cannot disagree. Across the corpus's 878 tables, every cell's text fills exactly its span rectangle in `html_table(header = FALSE)`. `html_tables(match =)` filters outermost tables by `grepl()` on their cleaned text, nested tables included, as pandas' `match=` does. `html_table_meta()` stays deferred.
...
Page metadata as built (roadmap Stage 11, `R/metadata.R`): `html_title()` follows `document.title`, taking the first `<title>` in the HTML namespace (an SVG `<title>` does not count) and stripping and collapsing ASCII whitespace; given a nodeset it uses the nodeset's document. `html_meta()` has the five columns above, attributes as written, and includes context nodes like `html_links()`. `html_images()`, `html_headings()`, `html_description()`, `html_canonical()` and `html_data()` stay deferred.
