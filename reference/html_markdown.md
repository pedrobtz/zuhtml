...
does, and Markdown-significant characters in it are escaped, so the text
...
- `<pre>` as a fenced code block, with its language when a `language-*`
  or `lang-*` class names one;
...
- links and images, their URLs resolved against the document's base URL
  ...
  does, or as written where that is not possible; `<a>` without `href`
  is plain text, and `<img>` without `src` is left out;
...
- data tables as GFM pipe tables: tables in which no cell spans rows or
  ...
  the header row, and cell contents stay on one line. Other tables, such
  as tables used for page layout, are written as their content, one
  paragraph per row, with cells separated by spaces.
...
#> # Release notes
#> 
#> Version **2.0** adds `fetch()`. See [the changes](https://example.org/docs/changes.html).
#> 
#> - Faster parsing
#> - New *options*:
#>   1. limits
