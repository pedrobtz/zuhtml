...
nodes selected by a subset of CSS, text, attributes, lists, tables and
links. It bundles the Gumbo HTML5 parser
...
| Allocation failure at any point is a clean, classed error that leaks nothing | `tools/run-sanitizers`: failure injected at every allocation index of the parser, the conversion, the serializer, the text cleaner and the table grid, under LeakSanitizer; `tests/testthat/test-seam.R` |
